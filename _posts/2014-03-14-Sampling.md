---
title: Sampling, particle system and neural network
layout: post
...
The quality is given by the difference : $|\frac1n \sum_{i=1}^n f(x_i) - \int f(x)\mathrm{d} \mu(x) |$.
...
Gradient estimation is the problem to calculate $ \nabla_{\eta} \mathbb{E}_{q_\eta}[f(x)]$ where $q_{\eta}$ is interpreted as a language model to predict the next token, $f$ is the reward model to evaluate the outcome of the prediction from the language model. It is then used to improve the model during a training process.
...
Anothere part of his talk was about blind source separation, e.g. separating indvidual voices from a noisy room, score matching, score network to diffusion model. Diffusion model was behind the explosive growth in image generative model in recent days.
